...
Mumble 不会为了加载 AU 插件而降低应用沙盒的安全性。macOS 版仅加载 AUv3 或明确声明为
`sandboxSafe` 的 Audio Unit，并优先交给系统的独立 AU 托管进程。

如果插件链显示“无法在不降低 Mumble 安全性的情况下加载”，说明当前安装的插件版本是
不兼容沙盒托管的旧式 AU。该插件会保留在插件链中，但不会加入音频处理链，因此应用启动时
也不会再出现“Lower Security Settings”系统弹窗。

**解决方案**：
1. 安装插件厂商提供的 AUv3 版本
2. 或更新到声明支持 App Sandbox / `sandboxSafe` 的 AUv2 版本
3. 重新打开 Mumble，并在 Mixer 中重新加载插件
