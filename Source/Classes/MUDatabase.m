...
static void *kMUDatabaseQueueKey = &kMUDatabaseQueueKey;
...
+ (void) ensureQueue {
    ...
    dispatch_once(&onceToken, ^{
        dbQueue = dispatch_queue_create("com.mumble.database.queue", DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(dbQueue, kMUDatabaseQueueKey, kMUDatabaseQueueKey, NULL);
    });
}
...
+ (void) performSync:(dispatch_block_t)block {
    ...
    [self ensureQueue];
    if (dispatch_get_specific(kMUDatabaseQueueKey)) {
        block();
    } else {
...
+ (NSMutableArray *) fetchAllFavourites {
    return [self performSyncReturningObject:^id{
...
+ (NSString *) digestForServerWithHostname:(NSString *)hostname port:(NSInteger)port {
    return [self performSyncReturningObject:^id{
        ...
        NSString *digest = nil;
        if ([result next]) {
            digest = [result stringForColumnIndex:0];
        }
        [result close];
        return digest;
    }];
